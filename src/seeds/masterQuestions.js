...
const seedMasterQuestions = async () => {
  try {
    ...
    for (let i = 0; i < 1000; i++) {
      const questionData = {
        srno: i,
        sp80053ControlNum: `AC-${faker.number.int({ min: 1, max: 53 })}`,
        controlName: faker.lorem.words(3),
        iso270012022CIdNum: `ISO-${faker.number.int({ min: 1, max: 100 })}`,
        ...
        vulnerabilityDesc: faker.lorem.sentences(2),
        ...
        operationalImpactDesc: faker.lorem.sentences(2),
        businessImpactDesc: faker.lorem.sentences(2),
        ...
        objAndProdOperImpactRating: faker.number.int({ min: 1, max: 10 }),
        ...
        revRiskLikelihoodRating: faker.number.int({ min: 1, max: 10 }),
        revRiskImpactRating: faker.number.int({ min: 1, max: 10 }),
